...
> ⚠️ `gets()` allows **unlimited input** → buffer overflow
...
**Meaning:** Stack addresses (0xbxxxxxxx) are **blocked**.
...
strdup copies to heap
  ↓
ret → jump to controlled address
```
...
- Prevents execution from the stack

**Bypass available:**
- Allows redirection to other memory regions (heap or libc)
